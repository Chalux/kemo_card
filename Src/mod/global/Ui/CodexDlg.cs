...
public partial class CodexDlg : BaseDlg
{
    ...
    protected override void InitEvent()
    {
        ...
        {
            Binder.OnTabChanged(_tabContainer, OnTabChanged);
        }

        if (_obField != null)
        {
            Binder.OnItemSelected(_obField, OnFieldSelected);
        }

        if (_obCharField != null)
        {
            Binder.OnItemSelected(_obCharField, OnCharFieldSelected);
        }
        ...
        {
            Binder.OnItemClicked(_itemListConditions, OnConditionItemClicked);
        }

        if (_itemListCharConditions != null)
        {
            Binder.OnItemClicked(_itemListCharConditions, OnCharConditionItemClicked);
        }

        if (_iptTxtFilter != null)
        {
            Binder.OnTextSubmitted(_iptTxtFilter, OnTextSubmitted);
        }

        if (_iptCharTxtFilter != null)
        {
            Binder.OnTextSubmitted(_iptCharTxtFilter, OnCharTextSubmitted);
        }

        if (_pager != null)
        {
            Binder.Bind(() => _pager.OnPageChanged = OnPagerPageChanged, () => _pager.OnPageChanged = null);
        }

        if (_charPager != null)
        {
            Binder.Bind(() => _charPager.OnPageChanged = OnCharPagerPageChanged, () => _charPager.OnPageChanged = null);
        }
    }
