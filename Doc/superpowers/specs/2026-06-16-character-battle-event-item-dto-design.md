...
- Character：`cards` → Card；`skillRefs` → Skill；`buffRefs` → Buff
...
| `ERace` | `[Flags]`：None, Human, Canine, Feline, Bird, Insect, Beast, Fish, Reptile, Plant, Machine, Demonic, Angel, Dragon, God, Devil, Undead, UnKnown |
...
| 类别 | 规则 |
|------|------|
| Character | `skillRefs` / `buffRefs` / `cards` 引用存在 |
