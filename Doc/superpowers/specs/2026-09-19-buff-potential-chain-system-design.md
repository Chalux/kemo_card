...
**关系**：服从 [2026-05-11 总规格](2026-05-11-kemo-card-design.md) 与 [2026-07-21 战斗规格](2026-07-21-combat-system-design.md)；**潜能一节替代总规格 §4.5.2–4.5.3 的旧模型**。
...
- **充能**（`slot.charge` tag）：实例参数 `charge`（罗马序号计数，充能 I = 1）。该槽每打出一张牌计数递减，归零触发 `onSlotCardPlayed` 载荷并**重置计数**（持续期内可反复触发）；到期移除前不因触发消失。chalux 专属卡「绝念」用同一机制投放**充能 II**（`charge: 2`，见 [充能球规格 §7](2026-09-19-charge-orb-system-design.md)）。
...
- 出牌是"标记 → CardExecution 统一结算"批处理：**结算阶段开始**按完整出牌队列一次性统计各属性的**不同角色数**（同一角色多张只计 1 人），档位作用于**本回合全部**该属性伤害/治疗卡——无次序、无首角色惩罚、无回溯。**只有连携适用的卡牌类型（Physics/Magical/Healing）参与人头统计**：控制/诅咒等非输出卡不把人头数堆进档位（统计侧与加成侧同规则）。
- 档位：**2 人 +25% / 3 人 +50% / 4 人 +100%**（1 人无增益；数值由 `ChainCalculator.TwoChainScale / ThreeChainScale / FourChainScale` 三个常量控制，2026-09-20 调档后写死在这三处，改档只改常量）。仅卡牌类型 Physics/Magical/Healing 适用；多属性卡取各属性最高档。
- 加成注入：结算单卡时设 `simulation.CurrentChainBonus`（try/finally 归零，卡牌上下文之外恒 0）；GAS 路径经 SetByCaller `ChainBonusScale`，直伤/治疗路径直接缩放。
- **加算规则**：连携与新属性 `DamageDealtScale`（全伤害增加）同桶：`伤害 = base × (1 + DamageDealtScale + 连携) × (1 + 目标受伤倍率)`——**三条伤害通道（GAS 公式 / 直伤定值 / 充能球）共用同一套缩放与伤害包管线**（2026-09-19 统一，见 [战斗规格 §1.3](2026-07-21-combat-system-design.md)「伤害包管线」），通道差异只在 `base` 怎么算。治疗 = `(amount + 源 HealPower) × (1 + 连携)`，不受 DamageDealtScale 影响。`MagicAttack` / `MagicDefense` 暂无消费公式（见 §7 后置项）。
...
`characters/chalux.json`：蓝 / Warrior / Canine，能量 8/3（角色定义级能量保底加算在卡组贡献之上），主动技蓄力链单档 [超限极寒 ×8]，卡组暂用 strike 占位。六条潜能被动（0/10/30/50/70/99）全部为 buff：

| 被动 | 实现 |
|---|---|
| P1 寒躯 | trait.immune_slot_damage tag |
| P2 寒火同源 | trait.chain_inject_red tag |
| P3 凛冬节拍 | onWaveStart + onTurnStart(turnInterval:8)：BoostSkillCounter 自身+4 / 蓝+2 / 犬+2（走 GainResource resource:skillcounter，叠加计算，超阈值丢弃由 GainSkillCounter 的 Cap 语义保证） |
| P4 极地血脉 | AllAllies + 条件(蓝或犬)：物攻/魔攻/治疗 +6 |
| P5 永冻威压 | AllAllies + 条件(蓝或犬)：DamageDealtScale +0.25 |
...
> **专属卡（2026-09-19）**：chalux 另有四张蓝属性 Epic 专属卡（逆戟冰冲 / 璨华长路 / 才煌的绝剑 / 绝念），已加入其初始卡组；其中「才煌的绝剑」授予充能球、「绝念」投放槽位充能 II。详见 [充能球系统规格 §7](2026-09-19-charge-orb-system-design.md)。

## 7. 明确后置项

- 联机表决网络同步（`IPotentialProposalApprover` 联机实现）
- 正式战斗界面（槽位 buff / 连携的玩家侧 UI；当前可视化在 RunDebugDlg"战斗检查"）
- chalux 正式卡组（占位 strike/strike_plus，等卡牌设计）
