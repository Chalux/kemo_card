...
> 给 Agent / 贡献者的项目地图。权威玩法细节以规格为准，本文不复制规格正文。  
...
| `Src/mod/` | 业务：`combat` / `run` / `global` 等 |
...
| `.cursor/rules/` | 始终生效的仓库约定 |
| `.cursor/skills/` | 项目 Agent skills |

依赖方向：**`mod` → `frame` / `utils` / `fixed`**，禁止反向。
...
- **组合优先于继承**：持有并委托，避免深继承。
- **本地化**：面向用户的文案必须用翻译键；场景 `text` 填键；C# 用 `Localization.Tr`。新增键写入 `Resource/Locale/strings.csv`（及 mod CSV）。日志可用明文。
- **不创建 `.uid` 文件**（引擎自动生成）。
- **连续大段同业务代码**（>5 个函数）用 `#region` / `#endregion`。
- **Git 提交说明**：简体中文（见 `git-commit-message.mdc`）。
- **缩进 / 编码风格**：以仓库根目录 [`.editorconfig`](../.editorconfig) 为权威（当前 C# 为 space / 4）。
- **格式化**：逻辑改完后只 format 本次改过的 `.cs`（`format-after-edit.mdc`）。
...
| Run 编排 | `Src/mod/run/` | 环模型、奖励、存档 |
| 全局 UI / 图鉴 / 设置 | `Src/mod/global/` | Menu、Codex、Card/Character UI 组件 |
| 内容管道 | `Src/frame/content/` | 发现、加载、合并、校验、注册表 |
...
- **更新本文件**：使用项目 skill [maintain-agent-doc](../.cursor/skills/maintain-agent-doc/SKILL.md)  
  触发：目录重组、新模块入口、权威规格增删、归档策略变化；日常小改不更新。
